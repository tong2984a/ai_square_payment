...
@app.get("/show_confirmation", response_class=HTMLResponse)
def read_root(request: Request, amount: str, currency: str, last_4: str, cardholder_name: str):
    return templates.TemplateResponse('confirmation.html', {'request': request, 'amount': amount, 'currency': currency, 'last_4': last_4, 'cardholder_name': cardholder_name})

@app.get("/diamond", response_class=HTMLResponse)
def read_root():
    return generate_index_html('diamond')

@app.get("/rocket", response_class=HTMLResponse)
def read_root():
    return generate_index_html('rocket')
